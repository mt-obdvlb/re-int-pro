...
用于计算机考研与复旦复试面试准备的 Agent 项目。已完成第 2 步候选题目调研及面试预演稿，等待用户验收，具体题目尚未确定。
...
| 2 | 调研并提供有理论依据、具备创新空间的 Agent 候选题目及面试讲稿 | 已交付，等待用户验收批准 |
...
第 2 步按总投入 150–300 小时、单个最终项目 API 预算不超过 500 元设计，不以训练或微调为前提。候选优先比较 ProbeOps（主动故障诊断）与 ClarifySQL（交互式数据库查询），最终由用户决定。讲稿全部为尚未实现时的预演稿，不能直接作为已完成项目经历使用。
