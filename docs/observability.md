...
`uv run --no-editable python scripts/trace.py <trace_id>` 可查看链路。当前没有接入 Jaeger/OTLP、日志按天清理、实时指标汇总、provider 重试或费用 uncertain 状态；不得将后文全量验收矩阵标记为已实现。P1 验证范围见 [验收记录](p1-validation.md)。
