...
单接口资源先实现 API-01～15 的独立请求断言；API-16～24 需要应用 fixture 和跨请求状态，作为 P1/P4 场景清单，不提前创建空场景冒充可运行用例。后续场景用 `import-steps --source test-case` 绑定现有用例，再编辑变量与断言。步骤顺序依 number；run_id 从创建响应传入，轮询≤180次且每秒一次，退出条件是终态。失败清理在隔离测试夹具完成，Agent 不持有重置接口。
